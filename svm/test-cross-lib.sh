#!/bin/bash

PRINTF=./svm/svm-features.exe
CROSS=./svm/or-test.js

DIR=$1

C=$2
G=$3

$PRINTF $DIR > /tmp/it.raw
sed \
  -e 's/null/1/g' \
  -e 's/yor/2/g' \
  -e 's/ai/3/g' \
  -e 's/odo/4/g' \
  -e 's/ika/5/g' \
  -e 's/haj/6/g' \
  -e '/^#/d' \
  /tmp/it.raw > /tmp/it.labeled
svm-scale /tmp/it.labeled > /tmp/it.scaled

cat /tmp/it.scaled | sort -R > /tmp/it.shuffled
lines=`wc -l /tmp/it.shuffled | awk '{print $1}'`
head -n $(( $lines * 9 / 10 )) /tmp/it.shuffled > /tmp/it.train
head -n $(( $lines * 1 / 10 )) /tmp/it.shuffled > /tmp/it.test

MODEL=/tmp/it.model
echo svm-train -s 0 -t 2 -c $C -g $G /tmp/it.train $MODEL
svm-train -s 0 -t 2 -c $C -g $G /tmp/it.train $MODEL
echo svm-predict /tmp/it.test $MODEL /tmp/result
svm-predict /tmp/it.test $MODEL /tmp/result

# GRIDTOOL=$HOME/Tools/libsvm-3.18/tools/grid.py
# result=`$GRIDTOOL -v 10 /tmp/it.scaled | tail -1`
# c=`echo $result | cut -d' ' -f1`
# g=`echo $result | cut -d' ' -f2`
# svm-train -t 2 -v 10 -c $c -g $g /tmp/it.scaled
